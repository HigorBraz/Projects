...
void Balls::update(float deltaTime, float speed) {
  for (auto &ball : m_balls) {
    ball.m_translation += ball.m_velocity * deltaTime * ((6 - speed) * 2);
...
Balls::Ball Balls::createBall(glm::vec2 translation, float scale) {
  ...
  std::uniform_real_distribution<float> rd(0.3f, 1.0f);
  const glm::vec4 color{rd(m_randomEngine), rd(m_randomEngine),
